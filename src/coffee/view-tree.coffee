...
#    000   000  000  00000000  000   000  000000000  00000000   00000000  00000000
#    000   000  000  000       000 0 000     000     000   000  000       000     
#     000 000   000  0000000   000000000     000     0000000    0000000   0000000 
#       000     000  000       000   000     000     000   000  000       000     
#        0      000  00000000  00     00     000     000   000  00000000  00000000
...
class ViewTree
    ...
    register: (node, cfg) ->
        ...
        if _.isNot(node.__i__) and cfg.__i__
    ...
    createView: (node, cfg) ->
        ...
        switch true
            when _.isSimple  cfg then cfg = @createTextView    node, node.cfg = text: cfg + ''
            when _.isDom     cfg then cfg = @createTagFromDom  node, null, cfg
            when _.isDomText cfg then cfg = @createTextFromDom node, null, cfg
            else
                tag = cfg.tag
                switch true
                    when _.isNot     tag then cfg = @createTextView    node, cfg
                    when _.isString  tag then cfg = @createTagView     node, cfg
                    when _.isDom     tag then cfg = @createTagFromDom  node, cfg, tag
                    when _.isDomText tag then cfg = @createTextFromDom node, cfg, tag
                    else
    ...
    createTextFromDom: (node, cfg, dom) ->
        ...
        cfg.tag = undefined
        #console.log 'createTextFromDom', node.text, node.view.nodeValue
        cfg
